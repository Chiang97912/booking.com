...
for i in range(n):
    ...
    tt.append(t)
    mm.append(m)
tt_s = '\n'.join([','.join([str(i) for i in t]) for t in tt])
mm_s = '\n'.join([','.join([str(i) for i in m]) for m in mm])

with open('data/mt.json', 'w') as f:
    json.dump(min_time_data, f)

with open('data/mm.json', 'w') as f:
    json.dump(min_money_data, f)
with open('data/mt.txt', 'w') as f:
    f.write(tt_s)
